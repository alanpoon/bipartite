...
//
	...
	function drawHeader(header, id,x){
	//	d3.select("#"+id).append("g").attr("class","header").append("text").text(header[2])
		...
		splitLine(header[2],headerSelect,'bold','font-weight',x,-20);
		...
		//	h.append("text").text(header[d]).attr("x", (dfg.c1[d]))
			...
			h.append("text").text("Count").attr("x", (dfg.c2[d]-10))
				.attr("y", -5).style("fill","grey");
	...
	bP.draw = function(data,cfgoptions){
		...
		data.forEach(function(biP,s){
			...
			drawHeader(biP.header, biP.id,window.dfgoptions.c3[2]/2);
